...
export class AuthService {
  ...
  addProductToWishList(product) {
    ...
    headers.append('Authorization', this.authToken);
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://localhost:3000/users/wishlist', product, { headers: headers })
      .map(res => res.json());
  }
  ...
  getOrderFromItems() {
    this.iteml = JSON.parse(localStorage.getItem("items"));
    return JSON.parse(localStorage.getItem("items"));
  }

  orderComplete(checkout) {
    localStorage.removeItem("items");
    let headers = new Headers();
    this.loadToken();
    ...
    headers.append('Authorization', this.authToken);
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://localhost:3000/users/orderplaced/' + checkout, { headers: headers })
      .map(res => res.json());
  }
